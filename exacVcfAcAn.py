...
def plotStuff(object):
    x1 = [0, 1, 2, 3, 4, 5, 6] #this is to position bars on barcharts for ACs
    x2 = [0, 1, 2, 3, 4, 5, 6] #this is to position bars on a separate figure, for ANs
    y1 = [4, 8, 10, 2, 3, 6, 7] #empty figure to be filled by allele's ACs
    y2 = [] #empty figure to be filled by allele's ANs
    plt.bar(x1, y1)
...
for record in vcf_reader:
    allele = Allele(record)
    subpop = SubPop(record)
    allele.pops.append(subpop)
    #recordList.append(allele)
